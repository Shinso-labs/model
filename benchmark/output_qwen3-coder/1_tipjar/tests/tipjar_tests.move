#[test_only]
module tipjar::tipjar_tests {
    use tipjar::tipjar::{Self, TipJar};
    ...
    fun test_init_creates_tipjar() {
        let mut scenario = ts::begin(@0xA);
        {
            tipjar::init_for_testing(ts::ctx(&mut scenario));
        };
    ...
    fun test_initial_tip_count_is_zero() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            let tipjar = ts::take_shared<TipJar>(&scenario);
            assert!(tipjar::get_tip_count(&tipjar) == 0, 1);
            ts::return_shared(tipjar);
        };
        ts::end(scenario);
    }
    ...
    fun test_initial_total_tips_is_zero() {
        ...
        {
            let tipjar = ts::take_shared<TipJar>(&scenario);
            assert!(tipjar::get_total_tips(&tipjar) == 0, 2);
            ts::return_shared(tipjar);
        };
        ts::end(scenario);
    }
    ...
    fun test_owner_is_set_correctly() {
        ...
        {
            let tipjar = ts::take_shared<TipJar>(&scenario);
            assert!(tipjar::get_owner(&tipjar) == @0xA, 3);
            ts::return_shared(tipjar);
        };
        ts::end(scenario);
    }
    ...
    fun test_send_single_tip() {
        ...
        {
            let mut tipjar = ts::take_shared<TipJar>(&scenario);
            let payment = ts::take_from_sender<Coin<SUI>>(&scenario);
            tipjar::send_tip(&mut tipjar, payment, ts::ctx(&mut scenario));
            assert!(tipjar::get_total_tips(&tipjar) == 100, 4);
            assert!(tipjar::get_tip_count(&tipjar) == 1, 5);
            ts::return_shared(tipjar);
        };
        ts::end(scenario);
    }
    ...
    fun test_send_multiple_tips() {
        ...
        ts::next_tx(&mut scenario, @0xC);
        {
            ...
            let payment = ts::take_from_sender<Coin<SUI>>(&scenario);
            tipjar::send_tip(&mut tipjar, payment, ts::ctx(&mut scenario));
            assert!(tipjar::get_total_tips(&tipjar) == 150, 6);
            assert!(tipjar::get_tip_count(&tipjar) == 2, 7);
            ts::return_shared(tipjar);
        };
        ts::end(scenario);
    }
    ...
    #[test]
    #[expected_failure(abort_code = tipjar::EInvalidTipAmount)]
    ...
    fun test_is_owner_returns_true() {
        ...
        {
            let tipjar = ts::take_shared<TipJar>(&scenario);
            assert!(tipjar::is_owner(&tipjar, @0xA), 9);
            ts::return_shared(tipjar);
        };
        ts::end(scenario);
    }
    ...
    fun test_is_owner_returns_false() {
        ...
        {
            let tipjar = ts::take_shared<TipJar>(&scenario);
            assert!(!tipjar::is_owner(&tipjar, @0xB), 10);
            ts::return_shared(tipjar);
        };
        ts::end(scenario);
    }
    ...
    fun test_tip_count_increments() {
        ...
        ts::next_tx(&mut scenario, @0xB);
        {
            ...
            let payment = ts::take_from_sender<Coin<SUI>>(&scenario);
            tipjar::send_tip(&mut tipjar, payment, ts::ctx(&mut scenario));
            assert!(tipjar::get_tip_count(&tipjar) == 3, 11);
            assert!(tipjar::get_total_tips(&tipjar) == 60, 12);
            ts::return_shared(tipjar);
        };
        ts::end(scenario);
    }
    ...
    fun test_large_tip_amount() {
        ...
        ts::next_tx(&mut scenario, @0xB);
        {
            ...
            let payment = ts::take_from_sender<Coin<SUI>>(&scenario);
            tipjar::send_tip(&mut tipjar, payment, ts::ctx(&mut scenario));
            assert!(tipjar::get_total_tips(&tipjar) == large_amount, 13);
            ts::return_shared(tipjar);
        };
        ts::end(scenario);
    }
