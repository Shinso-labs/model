...
module weather_oracle::weather_oracle_tests {
    use weather_oracle::weather::{Self, WeatherOracle, AdminCap, WEATHER};
    ...
    fun test_add_multiple_cities() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            let admin_cap = ts::take_from_sender<AdminCap>(&scenario);
            let mut oracle = ts::take_shared<WeatherOracle>(&scenario);
            weather::add_city(&admin_cap, &mut oracle, 1, b"City1", b"Country1", 10, true, 20, true, ts::ctx(&mut scenario));
            weather::add_city(&admin_cap, &mut oracle, 2, b"City2", b"Country2", 30, false, 40, false, ts::ctx(&mut scenario));
            weather::add_city(&admin_cap, &mut oracle, 3, b"City3", b"Country3", 50, true, 60, true, ts::ctx(&mut scenario));
            ts::return_shared(oracle);
            ts::return_to_sender(&scenario, admin_cap);
        };
        ts::end(scenario);
    ...
    fun test_remove_city() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            let admin_cap = ts::take_from_sender<AdminCap>(&scenario);
            let mut oracle = ts::take_shared<WeatherOracle>(&scenario);
            weather::add_city(&admin_cap, &mut oracle, 123, b"Test City", b"Test Country", 10, true, 20, true, ts::ctx(&mut scenario));
    ...
    fun test_update_city_weather() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            let admin_cap = ts::take_from_sender<AdminCap>(&scenario);
            let mut oracle = ts::take_shared<WeatherOracle>(&scenario);
            weather::add_city(&admin_cap, &mut oracle, 100, b"Test", b"Country", 10, true, 20, true, ts::ctx(&mut scenario));
            weather::update_city(
                &admin_cap,
                &mut oracle,
                100,
    ...
    fun test_get_city_name() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            ...
            weather::add_city(&admin_cap, &mut oracle, 999, b"Tokyo", b"Japan", 35, true, 139, true, ts::ctx(&mut scenario));
    ...
    fun test_get_city_country() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            ...
            weather::add_city(&admin_cap, &mut oracle, 500, b"Paris", b"France", 48, true, 2, true, ts::ctx(&mut scenario));
    ...
    fun test_weather_data_getters() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            let admin_cap = ts::take_from_sender<AdminCap>(&scenario);
            let mut oracle = ts::take_shared<WeatherOracle>(&scenario);
            weather::add_city(&admin_cap, &mut oracle, 200, b"City", b"Country", 10, true, 20, true, ts::ctx(&mut scenario));
    ...
    fun test_mint_weather_nft() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            let admin_cap = ts::take_from_sender<AdminCap>(&scenario);
            let mut oracle = ts::take_shared<WeatherOracle>(&scenario);
            weather::add_city(&admin_cap, &mut oracle, 300, b"NFT City", b"NFT Country", 25, true, 50, false, ts::ctx(&mut scenario));
    ...
    fun test_location_getters() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            let admin_cap = ts::take_from_sender<AdminCap>(&scenario);
            let mut oracle = ts::take_shared<WeatherOracle>(&scenario);
            weather::add_city(&admin_cap, &mut oracle, 400, b"Location Test", b"Test", 45, false, 90, true, ts::ctx(&mut scenario));
    ...
    fun test_update_with_wind_gust() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            let admin_cap = ts::take_from_sender<AdminCap>(&scenario);
            let mut oracle = ts::take_shared<WeatherOracle>(&scenario);
            weather::add_city(&admin_cap, &mut oracle, 600, b"Windy City", b"Country", 10, true, 20, true, ts::ctx(&mut scenario));
    ...
    fun test_add_city_edge_cases() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            ...
            weather::add_city(&admin_cap, &mut oracle, 700, b"Equator", b"Ocean", 0, true, 0, true, ts::ctx(&mut scenario));
            // Test with large geoname_id
            weather::add_city(&admin_cap, &mut oracle, 4294967295, b"Max ID", b"Test", 90, true, 180, true, ts::ctx(&mut scenario));
    ...
    fun test_multiple_updates() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            let admin_cap = ts::take_from_sender<AdminCap>(&scenario);
            let mut oracle = ts::take_shared<WeatherOracle>(&scenario);
            weather::add_city(&admin_cap, &mut oracle, 800, b"Changing City", b"Country", 10, true, 20, true, ts::ctx(&mut scenario));
    ...
    fun test_admin_cap_required() {
        ...
        ts::next_tx(&mut scenario, @0xA);
        {
            ...
            weather::add_city(&admin_cap, &mut oracle, 900, b"Admin Test", b"Test", 10, true, 20, true, ts::ctx(&mut scenario));
            ts::return_shared(oracle);
            ts::return_to_sender(&scenario, admin_cap);
        };
        ts::end(scenario);
